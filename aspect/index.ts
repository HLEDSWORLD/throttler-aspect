...
class Aspect implements IPreContractCallJP {
    ...
    preContractCall(input: PreContractCallInput): void {
        ...
        const contractAddress :string = unit8ArrayToHex(input.call!.to);
        const from :string = unit8ArrayToHex(input.call!.from);
        const storagePrefix :string = `${contractAddress}:${from}`;
        
        // load the current block timestamp
        const blockTimeBytes :Unit8Array = sys.hostApi.runtimeContext.get('block.header.timestamp');   
        const blockTime :number = Protobuf.decode<UnitData>(blockTimeBytes,UnitData.decode).data;
        
        // load last execution timestamp
        const lastExecState :MutableStateValue<number> = sys.aspect.mutableState.get<u64>(storagePrefix + 'lastExecAt');
